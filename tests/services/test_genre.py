...
def genre_dao():
    ...
    genre_dao.get_one = MagicMock(return_value={"id": 1, "name": "драма"})
    genre_dao.get_all = MagicMock(return_value= [{"id": 1, "name": "драма"}, {"id": 2, "name": "комедия"}])
    genre_dao.create = MagicMock(return_value={"id": 3, "name": "фантастика"})
    genre_dao.update = MagicMock(return_value={"id": 3, "name": "комедия"})
